...
#' @export
dimsumms__load_toxicity <- function(
  dms_dir,
  object=F,
  read_filter=T
  ){
  ...
  if(object){
    ...
    silent <- data.table::fread(file.path(dms_dir, "fitness_silent.txt"))
	  singles <- data.table::fread(file.path(dms_dir, "fitness_singles.txt"))
	  doubles <- data.table::fread(file.path(dms_dir, "fitness_doubles.txt"))
  }
  #Toxicity - silent
