...
dimsumms_bottleneck_fitness_scatter <- function(
  ...
  temp_cor <- plyr::ddply(plot_dt, c("bottleneck.y", "filter_status.y"), plyr::summarize, cor = round(cor(fitness.x, fitness.y, use = "pairwise.complete"), 2), n = length(fitness.x))
    ...
    ggplot2::scale_fill_gradientn(colours = c("white", "black"), trans = "log10") +
	  ...
	  ggplot2::theme_bw() +
	  ggplot2::facet_grid(filter_status.y ~ bottleneck.y, scales="free") +
